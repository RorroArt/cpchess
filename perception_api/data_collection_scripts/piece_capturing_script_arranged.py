...
# Augmentations (ported from collect_piece_horizontals_prompted_aug.py)
AUG_ROTATIONS       = True
AUG_FLIPS           = True
...
def generate_augmented_variants(img_bgr: np.ndarray):
    """Return list of (variant_bgr, suffix) with rotations, flips, and brightness/contrast jitters."""
    variants = []
    base = [("orig", img_bgr)]
    if AUG_ROTATIONS:
        base.extend([
...
# ---------- Helpers ----------
PIECE_ORDER_ARRANGED = ["rook","knight","bishop","queen","king","bishop","knight","rook"]
...
def main():
    ensure_meta_exists()
    print("=== Row Batch Collector (Preview + Confirm, with Augmentations) ===")
    print("I will capture the whole board, annotate one horizontal row, and (on confirm) save augmented crops + embeddings.")

    # Ask row: A..H (top→bottom to match split_grid)
    row_letter = ask("Which row to capture? [A-H] (A=top, H=bottom): ", set(list("abcdefgh"))).upper()

    # Layout: arranged or pawns
    layout = ask("Layout type? [arranged/pawns]: ", {"arranged","pawns"})

    # Color: w/b
    color = ask("Pieces color? [w/b]: ", {"w","b"})
    color_full = "white" if color == "w" else "black"

    # Build intended labels for the 8 squares in this row
    files = [str(i) for i in range(1,9)]
    piece_names = PIECE_ORDER_ARRANGED if layout == "arranged" else ["pawn"]*8
    target_squares = [f"{row_letter}{f}" for f in files]
    labels_by_square = {sq: f"{color_full}_{pname}" for sq, pname in zip(target_squares, piece_names)}
    ...
    # === Pre-analysis transforms: mirror horizontally, then rotate 90° CCW ===
    warp = cv2.flip(warp, 1)  # horizontal mirror
    warp = cv2.rotate(warp, cv2.ROTATE_90_COUNTERCLOCKWISE)

    sq_dict, rects = split_grid(warp)

    # Sanity: ensure all requested squares exist
    missing = [s for s in target_squares if s not in sq_dict]
    if missing:
        print(f"[ERROR] Squares not recognized: {missing}")
        return

    # Draw preview and ask for confirmation
    preview = draw_labels_overlay(warp, rects, labels_by_square)
    cv2.imshow("Row Batch Preview (Press any key when ready)", preview)
    cv2.waitKey(0)

    confirm = ask("Confirm saving embeddings for these 8 squares (with augmentation)? [y/n]: ", {"y","n"})
    if confirm == "n":
        print("Cancelled. No embeddings saved.")
        cv2.destroyAllWindows()
        return

    # Save crops, embeddings, and metadata
    ts_base = datetime.now().strftime("%Y%m%d_%H%M%S_%f")

    saved = 0
    variants_per_square = None
    for sq, pname in zip(target_squares, piece_names):
        crop, sq_idx = sq_dict[sq]
        label = f"{color_full}_{pname}"
        piece_code = PIECE_CODE[pname]
    ...
    print(f"Saved {saved} augmented embeddings for row {row_letter} using layout '{layout}' ({color_full}). ~{variants_per_square} variants per square × 8 squares.")
    print(f"Images stored under: {OUT_DIR}")

    # Show final preview briefly
